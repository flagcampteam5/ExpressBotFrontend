...
class Request extends Component {
  constructor(props) {
    ...
    this.onFoundStation = this.onFoundStation.bind(this);
    this.state = {
      orderPlaced: false,
      customerEmail: null,
      recipientEmail: null,
    ...
    return (
      <div>
        <Confirmation
          ...
          isRobot={this.state.typeOfRobot === "RoadBot"}
    ...
    return (
      <div className="MapContainer">
        <Map
          pickUpLatLng={this.state.pickUpLatLng}
          destinationLatLng={this.state.destinationLatLng}
          onFoundStation={this.onFoundStation}
          ref={this.Map}
        />
      </div>
    );
  };
  onFoundStation = (number) => {
    this.setState({
      stationId: number,
    });
    console.log("found closest station", number);
  };
  handleSubmit = () => {
    var time = new Date();
